...
import { IMPORT_STAGE_OPTIONS, type ScrapedResult } from './scSearchTypes'
import type { PipelineStage } from '@/types'

type ScSearchReviewProps = {
...
export function ScSearchReview({
  scrapedResults,
  toggleScrapedSelect,
  selectAllWithEmails,
  deselectAllScraped,
...
}: ScSearchReviewProps): React.JSX.Element {
  return (
              ...
              >
                ...
                <td className="px-4 py-2.5">
                  <div className="flex items-center gap-3">
                    {artist.image_url || artist.match?.avatar_url ? (
                      <img
                        src={artist.image_url || artist.match?.avatar_url || ''}
                        alt=""
                      ...
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-gray-200 to-gray-300 text-xs font-bold text-gray-500">
                        {(artist.match?.name ?? artist.query).charAt(0).toUpperCase()}
                      </div>
                    )}
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-gray-900">
                        {artist.match?.name ?? artist.query}
                      </div>
                      {artist.isDuplicate ? (
                        ...
                        artist.query !== artist.match?.name && (
                ...
                <td className="px-4 py-2.5 text-right font-mono text-xs text-gray-500">
                  {artist.match?.followers?.toLocaleString() ?? '-'}
                </td>
                <td className="px-4 py-2.5">
                  <div className="flex gap-1">
                    {artist.match?.url && (
                      <a
                        href={artist.match.url}
                        target="_blank"
                    ...
                    {artist.spotify_url && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex h-6 w-6 items-center justify-center rounded-md bg-emerald-50 text-[10px] font-bold text-emerald-600 transition-colors hover:bg-emerald-600 hover:text-white"
                        title="Spotify"
                      >
                        SP
                      </a>
