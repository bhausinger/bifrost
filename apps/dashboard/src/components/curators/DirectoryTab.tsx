      ...
      <div className="card mb-4 p-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-400">
            {filtered?.length ?? 0} curator{(filtered?.length ?? 0) !== 1 ? 's' : ''}
          </span>
        </div>
      </div>
      ...
      {!isLoading && filtered?.length === 0 && (
        <div className="card p-12 text-center">
          ...
          <p className="text-sm font-medium text-gray-400">
            No curators yet. Add your first curator to start building your network.
          </p>
        </div>
      )}
...
function DirectoryStats({
  ...
  const totalReach =
    curators?.reduce((s, c) => s + c.playlists.reduce((ps, p) => ps + p.follower_count, 0), 0) ?? 0
  const reachDisplay =
  ...
  return (
    <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-3">
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-gray-900">{activeCurators}</p>
          <p className="mt-0.5 text-sm font-medium text-gray-400">Active Curators</p>
          <p className="text-xs text-gray-400">{curators?.length ?? 0} total</p>
        </div>
      ...
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          ...
          <p className="mt-0.5 text-sm font-medium text-gray-400">Total Playlists</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
          <ListMusic className="h-6 w-6 text-violet-600" />
        </div>
      </div>
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-gray-900">{reachDisplay}</p>
          <p className="mt-0.5 text-sm font-medium text-gray-400">Total Reach</p>
          <p className="text-xs text-gray-400">combined followers</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
...
function CuratorCard({ curator, onClick }: { curator: CuratorWithPlaylists; onClick: () => void }) {
  ...
  const genres = curator.genres?.length
    ? curator.genres
    : [...new Set(curator.playlists.map((p) => p.genre).filter(Boolean))]
  const totalFollowers = curator.playlists.reduce((s, p) => s + p.follower_count, 0)

  return (
    <div
      onClick={onClick}
      className="card cursor-pointer p-5 transition-all duration-200 hover:border-gray-300"
    >
      <div className="flex items-start gap-4">
        <div
          className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${gradient} shadow-md`}
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className="truncate text-sm font-bold text-gray-900">{curator.name}</h3>
            <span
              className={`h-2 w-2 rounded-full flex-shrink-0 ${curator.is_active ? 'bg-emerald-500' : 'bg-gray-300'}`}
            />
          </div>
          {curator.contact_name && (
            <p className="truncate text-xs text-gray-400 mt-0.5">{curator.contact_name}</p>
          )}
          {!curator.contact_name && curator.email && (
            <p className="truncate text-xs text-gray-400 mt-0.5">{curator.email}</p>
          )}
        </div>
      </div>
      <div className="mt-4 flex items-center gap-4 text-xs text-gray-400">
        <span className="flex items-center gap-1">
          ...
          {curator.playlists.length.toLocaleString()} playlists
        </span>
        <span className="flex items-center gap-1">
        ...
        {curator.price_per_10k !== null && (
          <span className="flex items-center gap-1 font-mono font-medium text-emerald-600">
            <DollarSign className="h-3.5 w-3.5" />${curator.price_per_10k}/10K
          </span>
        )}
      </div>
      {genres.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {genres.slice(0, 4).map((g) => (
            <span
              key={g}
              className="inline-flex items-center rounded-md bg-violet-50 px-1.5 py-0.5 text-[10px] font-medium text-violet-600 ring-1 ring-inset ring-violet-200/60"
            >
              {g}
            </span>
          ))}
          {genres.length > 4 && (
            <span className="text-[10px] text-gray-400 self-center">+{genres.length - 4} more</span>
          )}
        </div>
      )}
      {curator.payment_method && (
        <div className="mt-3 flex items-center gap-1.5 text-xs text-gray-400">
          ...
          {curator.payment_method}
          {curator.payment_handle && ` — ${curator.payment_handle}`}
        </div>
      )}
    </div>
  )
