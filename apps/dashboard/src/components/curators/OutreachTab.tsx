  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((e) => {
      if (search) {
        ...
        if (
          !e.playlist_name.toLowerCase().includes(q) &&
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="p-12 text-center">
            ...
            <p className="text-sm font-medium text-gray-400">
              {entries?.length === 0
                ? 'No playlists tracked yet. Add your first one to start tracking outreach.'
...
function OutreachStats({
  ...
  stats: { total: number; emailed: number; replied: number; confirmed: number }
}) {
  return (
    <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-gray-900">{stats.total}</p>
      ...
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-blue-600">{stats.emailed}</p>
          <p className="mt-0.5 text-sm font-medium text-gray-400">Emailed</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
      ...
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-violet-600">{stats.replied}</p>
          <p className="mt-0.5 text-sm font-medium text-gray-400">Replied</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
      ...
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-emerald-600">{stats.confirmed}</p>
          <p className="mt-0.5 text-sm font-medium text-gray-400">Confirmed</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
