  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50/80 p-6">
        ...
        />
        <PlaylistTable playlists={filteredPlaylists} totalCount={curator.playlists.length} />
      </div>
    </div>
  )
}
      ...
      <div className="flex items-start gap-5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3">
            ...
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${curator.is_active ? 'bg-emerald-50 text-emerald-600 ring-1 ring-inset ring-emerald-600/20' : 'bg-gray-100 text-gray-400 ring-1 ring-inset ring-gray-300/60'}`}
            >
              <span
                className={`h-1.5 w-1.5 rounded-full ${curator.is_active ? 'bg-emerald-500' : 'bg-gray-300'}`}
              />
          ...
          <div className="mt-2 flex flex-wrap items-center gap-4 text-sm text-gray-500">
            {curator.contact_name && (
              <span className="font-medium text-gray-700">{curator.contact_name}</span>
            )}
          ...
          {curator.genres?.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {curator.genres.map((g) => (
                <span
                  key={g}
                  className="inline-flex items-center rounded-md bg-violet-50 px-2 py-0.5 text-xs font-medium text-violet-600 ring-1 ring-inset ring-violet-200/60"
                >
                  {g}
                </span>
              ))}
          ...
          {curator.notes && (
            <p className="mt-2 text-sm text-gray-400 leading-relaxed">{curator.notes}</p>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          ...
          <p className="mt-0.5 text-sm font-medium text-gray-400">Playlists</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
          <ListMusic className="h-6 w-6 text-teal-500" />
        </div>
      </div>
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-gray-900">{genreCount}</p>
          <p className="mt-0.5 text-sm font-medium text-gray-400">Genres</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
      ...
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-gray-900">{countryCount}</p>
          <p className="mt-0.5 text-sm font-medium text-gray-400">Countries</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
      ...
      <div className="kpi-card card flex items-center justify-between p-5">
        <div>
          <p className="font-mono text-2xl font-bold text-emerald-600">{priceDisplay}</p>
          <p className="mt-0.5 text-sm font-medium text-gray-400">Per 10K</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-white">
      ...
      {playlists.length === 0 ? (
        <div className="p-12 text-center">
          ...
          <p className="text-sm font-medium text-gray-400">
            {totalCount === 0 ? 'No playlists added yet.' : 'No playlists match your filters.'}
          </p>
        </div>
      ) : (
          ...
          <tbody className="divide-y divide-gray-100">
            {playlists.map((p) => (
              <tr key={p.id} className="table-row transition-colors hover:bg-gray-50">
                ...
                <td className="px-5 py-3.5">
                  {p.price_per_placement !== null ? (
                    <span className="font-mono text-sm font-medium text-emerald-600">
                      ${p.price_per_placement}
                    </span>
                  ) : (
                    <span className="text-sm text-gray-400">-</span>
                  )}
                ...
                <td className="px-5 py-3.5 font-mono text-sm text-gray-500">
                  {p.avg_streams_per_placement !== null
                    ? `~${p.avg_streams_per_placement.toLocaleString()}`
                    : '-'}
                </td>
              </tr>
            ))}
